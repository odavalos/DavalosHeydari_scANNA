...
sample_no <- 'ImmuneCSF' # change accordingly
...
# load dataset
immunecsf <- LoadH5Seurat(file = '/Volumes/BigDrive/GradSchool/NACT_NewDatasets/GSE163005_IMMUNE_CSF/GSE_ImmuneCSF_qc_hvg_anno_5k.h5Seurat')

# read in metadata that contains split column
metadata <- read_csv(file = '/Volumes/BigDrive/NACT_ComparisonModels/DatasetSplits/Metadata_Splits/GSE_ImmuneCSF_metadata_splits.csv')
...
immunecsf <- DietSeurat(immunecsf, 
           counts = TRUE, 
           data = TRUE, 
           scale.data = FALSE)

# # a function to run through the scClassify process
runSplitscClassify <- function(seuratdata, 
                               split, 
  ...
  scClassify_res <- scClassify(exprsMat_train = reference@assays$RNA@data,
                               cellTypes_train = reference@meta.data[[cluster_col]],
                               exprsMat_test = list(query = query@assays$RNA@data),
                               cellTypes_test = list(query = query@meta.data[[cluster_col]]),
                               tree = "HOPACH",
                               algorithm = "WKNN",

...
# load dataset
GSE144236 <- LoadH5Seurat(file = '~/Documents/NACT_Datasets/GSE144236_qc_hvg_anno_5k.h5Seurat')

# read in metadata that contains split column
metadata <- read_csv(file = '~/Documents/NACT_Datasets/Metadata_Splits/GSE144236_metadata_splits.csv')
...
for(i in split_vect){
  ...
  res_list <- try(runSplitCHETAH(seuratdata = GSE144236, split = i))

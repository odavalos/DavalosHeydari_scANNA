...
# load dataset
lung <- LoadH5Seurat(file = '~/Documents/NACT_Datasets/Lukassen2020_Lung_qc_hvg_anno_5k.h5Seurat')

# read in metadata that contains split column
metadata <- read_csv(file = '~/Documents/NACT_Datasets/Metadata_Splits/Lukassen2020_Lung_metadata_splits.csv')
...
for(i in split_vect){
  ...
  res_list <- try(runSplitCHETAH(seuratdata = lung, split = i))

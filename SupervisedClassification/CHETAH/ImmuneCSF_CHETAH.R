...
# load dataset
immunecsf <- readRDS(file = '~/Documents/NACT_Datasets/GSE_ImmuneCSF_qc_hvg_anno_5k.rds')

# read in metadata that contains split column
metadata <- read_csv(file = '~/Documents/NACT_Datasets/Metadata_Splits/GSE_ImmuneCSF_metadata_splits.csv')
...
runSplitCHETAH <- function(seuratdata, split, cluster_col = 'general_celltypes', dataset = sample_no){
...
for(i in split_vect){
  ...
  res_list <- try(runSplitCHETAH(seuratdata = immunecsf, split = i))

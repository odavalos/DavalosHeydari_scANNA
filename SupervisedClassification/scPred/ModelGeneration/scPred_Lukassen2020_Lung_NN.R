...
# load dataset
lung <- LoadH5Seurat(file = '~/Lukassen2020_Lung/Lukassen2020_Lung_qc_hvg_anno_5k.h5Seurat')

# read in metadata that contains split column
metadata <- read_csv(file = '~/DatasetSplits/Metadata_Splits/Lukassen2020_Lung_metadata_splits.csv')
...
runSplitscPred <- function(seuratdata, split, cluster_col = 'celltypes', mod='nnet'){
...
for(i in split_vect){
  ...
  repeat {
    res_list <- try(runSplitscPred(seuratdata = lung, split = i))

...
# load dataset
scp1361 <- LoadH5Seurat(file = '~/SCP1361/scp1361_qc_hvg_anno_5k.h5Seurat')

# read in metadata that contains split column
metadata <- read_csv(file = '~/DatasetSplits/Metadata_Splits/SCP1361_metadata_splits.csv')
...
runSplitscPred <- function(seuratdata, split, cluster_col = 'celltypes', model='svmRadial'){
  message(crayon::yellow(paste0("Running scPred model: ", model, ' on split: ', split)))
...
for(i in split_vect){
  #print(as.name(i))
  res_list <- runSplitscPred(seuratdata = scp1361, split = i)

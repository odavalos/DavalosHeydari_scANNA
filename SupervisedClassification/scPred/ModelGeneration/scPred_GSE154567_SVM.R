...
# load dataset
gse154567 <- LoadH5Seurat(file = '~/GSE154567/GSE154567_qc_hvg_anno_5k_raw.h5Seurat')

# read in metadata that contains split column
metadata <- read_csv(file = '~/DatasetSplits/Metadata_Splits/GSE154567_metadata_splits.csv')
...
colnames(gse154567@reductions$umap@cell.embeddings) <- c('UMAP_1', 'UMAP_2')
...
runSplitscPred <- function(seuratdata, split, cluster_col = 'celltypes', mod='svmRadial'){
...
for(i in split_vect){
  ...
  repeat {
    res_list <- try(runSplitscPred(seuratdata = gse154567, split = i))

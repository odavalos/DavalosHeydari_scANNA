...
# load dataset
lung <- LoadH5Seurat(file = '/Volumes/BigDrive/GradSchool/NACT_NewDatasets/Lukassen2020_Lung/Lukassen2020_Lung_qc_hvg_anno_5k.h5Seurat')

# read in metadata that contains split column
metadata <- read_csv(file = '/Volumes/BigDrive/NACT_ComparisonModels/DatasetSplits/Metadata_Splits/Lukassen2020_Lung_metadata_splits.csv')
...
table(rownames(lung@meta.data) == metadata$barcodes)

# append 5 split columns to the seurat metadata df
lung@meta.data <- cbind(lung@meta.data, metadata[,c("Split_1","Split_2", "Split_3", "Split_4", "Split_5")])

split_vect <- c("Split_1","Split_2", "Split_3", "Split_4", "Split_5")


lung <- DietSeurat(lung, 
                   counts = TRUE, 
                   data = TRUE, 
                   scale.data = FALSE,
...
runSplitSCN <- function(seuratdata, split, cluster_col = 'celltypes', dataset = sample_no){
  ...
  # model evaluation
  cm <- caret::confusionMatrix(query@meta.data$singleCellNet_pred,
...
for(i in split_vect){
  ...
  res_list <- try(runSplitSCN(seuratdata = lung, split = i))
  # repeat {

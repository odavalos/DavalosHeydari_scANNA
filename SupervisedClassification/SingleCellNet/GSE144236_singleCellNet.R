...
# load dataset
GSE144236 <- LoadH5Seurat(file = '/Volumes/BigDrive/GradSchool/NACT_NewDatasets/GSE144236/GSE144236_qc_hvg_anno_5k_raw.h5Seurat')

# read in metadata that contains split column
metadata <- read_csv(file = '/Volumes/BigDrive/NACT_ComparisonModels/DatasetSplits/Metadata_Splits/GSE144236_metadata_splits.csv')
...
GSE144236 <- DietSeurat(GSE144236, 
                        counts = TRUE, 
                        data = TRUE, 
                        scale.data = FALSE,
...
for(i in split_vect){
  ...
  res_list <- try(runSplitSCN(seuratdata = GSE144236, split = i))
  # repeat {
